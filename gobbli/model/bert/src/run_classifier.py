...
Minor modifications to support gobbli interface. """
...
class ColaProcessor(DataProcessor):
    ...
    """gobbli: Repurposed for arbitrary multiclass classification."""

    def get_train_examples(self, data_dir):
        """See base class."""
        train_file = os.path.join(data_dir, "train.tsv")
        if os.path.exists(train_file):
            self._train_examples = self._create_examples(
    ...
    def get_dev_examples(self, data_dir):
        ...
        if os.path.exists(dev_file):
            self._dev_examples = self._create_examples(self._read_tsv(dev_file), "dev")
    ...
    def get_test_examples(self, data_dir):
        ...
        if os.path.exists(test_file):
            self._test_examples = self._create_examples(
    ...
    def _create_examples(self, lines, set_type):
        """Creates examples for the training and dev sets."""
        # Choose an arbitrary label for the test dataset
        # It doesn't matter what it is; it just needs to be one of the ones
        # present in our training/validation set
        ...
        for (i, line) in enumerate(lines):
            ...
            if set_type == "test":
                ...
                if label not in self._labels:
                    raise ValueError(
                        "label {} not found in set of labels {}".format(
                            label, self._labels
                        )
                    )
            examples.append(
